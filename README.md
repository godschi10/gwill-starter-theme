...
**Latest version: 1.12.1** - Section C of the UI makeover list: post lists gain card chrome (Customizer: Content Lists - plain / cards / horizontal / grid; cards is the default), hover affordances, category chips, and an archive-header band with a count pill. Full history in `CHANGELOG.md`.
...
A complete no-plugin SEO layer (ported from the GWill Finance theme, v1.2.0, adapted to a generic base - the same approach as the tech and finance themes). **Everything defers to a major SEO plugin (RankMath, Yoast, AIOSEO, SEOPress, The SEO Framework) when one is active, via `gwill_seo_plugin_active()`** - all ten theme-owned SEO outputs, completed in v1.3.1 by the conflict audit (front-page title + sitemap rewrite/template/canonical guards):

- **Front-page `<title>` capped at ~60 chars** - `gwill_front_page_title()` builds "Brand - Tagline" and trims the tagline word-by-word to fit the budget (filterable headline: `gwill_front_page_tagline`); plugin-guarded since v1.3.1 (RankMath hooks `pre_get_document_title` at 10, this filter at 11 - the guard stops the theme overriding a configured homepage title). Long-titled singulars drop the brand suffix from `<title>` when the full string would exceed 65 chars (`gwill_document_title_parts`).
...
- **Theme-owned `/sitemap.xml`** (`inc/sitemap.php` + root `sitemap.php`) - rewrite rule flushed by the version-keyed `gwill_maybe_flush_rewrites()`, transient-cached XML invalidated on `save_post`, includes posts/pages/public CPTs, excludes `gwill_hidden_slugs()` pages. The rewrite, template swap and canonical suppression are all plugin-guarded since v1.3.1 (AIOSEO / The SEO Framework both serve their main sitemap at `/sitemap.xml` - the theme must never hijack it).
