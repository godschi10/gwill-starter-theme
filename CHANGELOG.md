...
- " - " collapse (2,835 lines); CHANGELOG 1.10.21 entry moved to top; residue flattened (.editorconfig, .gitignore); guard pin 3.20.5.

## [1.10.21] - 2026-09-05

### Fleet-wide em-dash removal (writing-voice pass)

- Tokenizer-grade em-dash removal across all PHP comments/docblocks, JS comments, CSS comments, and docs (King decree: zero em-dashes fleet-wide).
- php -l / node --check green on every touched file. Version bump only - no behavioral change.
...
- **Version guard** - `$expected` in `inc/enqueue.php` 3.15.0 → **3.16.0**. Plugin v3.16.0 added in-thread search (server-side whole-thread LIKE with reply-context chips, cache-before-ratelimit, local-filter fallback). The enqueue gate + dark-state coverage are untouched.

**Verified:** `php -l` clean; battery 5/5 re-run at 3.16.0.
...
- **Version guard** - `$expected` in `inc/enqueue.php` 3.14.0 → **3.14.1**. The plugin's v3.14.1 fixed the @mention dropdown positioning (King-reported: `position:fixed` element was positioned with page coordinates `rect.bottom + scrollY`, rendering it off-screen on any scrolled page). The enqueue gate + dark-state coverage are untouched.

**Verified:** `php -l` clean; battery 5/5 re-run at 3.14.1.
...
- **Version guard** - `$expected` in `inc/enqueue.php` 3.8.0 → **3.10.0**. The plugin's v3.9.0/3.10.0 shipped reply-email notifications (wp_mail rail) and the Comment Analytics Dashboard - all plugin-side; the enqueue gate and dark-state coverage are untouched.
